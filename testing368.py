...
def initializeMovies():
    ...
    with open("CSE368FinalProject/u.item") as file:
...
def initializeUsers():
    ...
    with open("CSE368FinalProject/u.user") as file:
...
def initializeRatings(movieBase, userBase):
    ...
    RatingBase = []
    with open("CSE368FinalProject/u.data") as file:
        allLines = file.readlines()
        for line in allLines:
...
if __name__ == "__main__":
    ...
    ratings = pd.read_csv("CSE368FinalProject/u.data", delimiter="\t")
    users = pd.read_csv("CSE368FinalProject/u.user",delimiter="|")
